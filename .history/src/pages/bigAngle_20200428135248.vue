  ...
  methods: {
    ...
    beforeUploadFile2(e) {
      ...
      this.angleImg = [
        {
          id: Math.floor(Math.random * 100),
          src: window.BASEURL.fliename +  `/static/data/${startName}/${startName}_angle.jpg`
        }
      ];
    ...
    INIT(width, height, name) {
      ...
      var modelPath = window.BASEURL.fliename +  `/static/data/${name}`;
    ...
    getModelTxt(name) {
      ...
      axios
        .get(window.BASEURL.fliename + `/static/data/${name}/face_points_distance.txt`)
        .then(function(res) {
    ...
    getInfo(name) {
      let VM = this;
      this.$http.api
        .get(window.BASEURL.fliename + `/static/data/${name}/${name}.json`)
        .then(function(res) {
          if (res.data.code === 0) {
